...
const buildPrompt = (request) => {
  ...
  return [
    ...
    '- mixControls is executed deterministically by the native audio engine during the transition',
    '- Keep gain/EQ/filter/loudness/clip-protection moves conservative and bounded by the schema',
    '- Use soft_limit only when the available peak evidence supports the requested ceiling; otherwise use monitor_only',
    '- Do not request realtime AI control, stem separation, vocal detection, key detection, or new LUFS analysis',
    '',
