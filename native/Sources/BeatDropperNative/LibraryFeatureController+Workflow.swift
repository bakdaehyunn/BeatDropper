...
extension LibraryFeatureController {
    func newSet() {
        let urls = trackImporter.openAudioFiles()
        ...
        Task {
            let imported = await trackImporter.importTracks(from: urls)
    ...
    func importFolder() {
        guard let folderURL = trackImporter.openMusicFolder() else {
            notice = "Canceled"
            return
        }
        ...
        Task {
            let imported = await trackImporter.importFolder(folderURL)
    ...
    @discardableResult
    func openExternalItems(_ urls: [URL], sourceName: String) async -> Int {
        let selection = trackImporter.classifyOpenURLs(urls)
    ...
    private func openExternalItemsAsSet(_ urls: [URL], sourceName: String) {
        Task {
            await openExternalItems(urls, sourceName: sourceName)
        }
    }

    @discardableResult
    func importFolder(at folderURL: URL) async -> Int {
        let imported = await trackImporter.importFolder(folderURL)
        guard !imported.isEmpty else {
            notice = "No supported audio files"
            return 0
        ...
        replacePlaylist(with: imported, sourceFolderURL: folderURL)
        notice = "Imported \(imported.count) tracks"
    ...
    private func replaceSetFromOpenSelection(_ selection: NativeOpenImportSelection, sourceName: String) async -> Int {
        ...
        let importedFiles = await trackImporter.importTracks(from: selection.audioFileURLs)
    ...
    func relinkSelectedTrack() {
        ...
        guard let replacementURL = trackImporter.openReplacementAudioFile() else {
            notice = "Canceled"
            return
        }
        ...
        Task {
            guard let replacement = await trackImporter.importTrack(from: replacementURL) else {
