...
extension LibraryFeatureController {
    func restoreLibraryState() {
        do {
            ...
            userPlaylistNameDraft = library.selectedUserPlaylist?.name ?? ""
    ...
    func fileFingerprint(for imported: ImportedTrack) -> String {
        let fileSize = trackImporter.fileSize(at: imported.url)
    ...
    func selectFirstAvailableTrackIfNeeded() {
        if let selectedTrack = library.selectedTrack, library.isAvailable(selectedTrack) {
            return
        }
        selectedTrackID = playlist.first(where: library.isAvailable)?.id ?? playlist.first?.id
    }
}
