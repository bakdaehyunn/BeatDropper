...
@MainActor
final class MixReviewFeatureController: MixReviewFeatureActionHandling {
    private let feature: MixReviewFeature
    private let library: LibraryFeature
    private let planning: MixPlanningFeature
    private let shell: AppShellFeature
    private let settingsActions: SettingsFeatureController
    private let artifactStore: any MixReviewRepository
    private let fileAvailability: any FileAvailabilityChecking
    private let clock: any AppClock

    init(
        feature: MixReviewFeature,
        library: LibraryFeature,
        planning: MixPlanningFeature,
        shell: AppShellFeature,
        settingsActions: SettingsFeatureController,
        artifactStore: any MixReviewRepository,
        fileAvailability: any FileAvailabilityChecking,
        clock: any AppClock
    ) {
    ...
    private var recentMixReviewEvents: [PlannedMixReviewEvent] {
        get { feature.recentEvents }
        set { feature.recentEvents = newValue }
    }
    private var importedMixReviewArtifacts: [ImportedMixReviewArtifact] {
        get { feature.importedArtifacts }
        set { feature.importedArtifacts = newValue }
    }
    private var isImportedMixReviewArtifactPairFilterEnabled: Bool {
        get { feature.isPairFilterEnabled }
        set { feature.isPairFilterEnabled = newValue }
    }
    private var importedMixReviewArtifactSearchText: String {
        get { feature.searchText }
        set { feature.searchText = newValue }
    }
    private var selectedImportedMixReviewComparisonArtifactIds: [ImportedMixReviewArtifact.ID] {
        get { feature.selectedComparisonArtifactIDs }
        set { feature.selectedComparisonArtifactIDs = newValue }
    }

    func record(_ event: PlannedMixReviewEvent) {
        recentMixReviewEvents.insert(event, at: 0)
        recentMixReviewEvents = Array(recentMixReviewEvents.prefix(12))
    }

    private var mixReviewArtifactFolderURL: URL? {
        guard let path = shell.settings.mixReviewArtifactFolderPath else { return nil }
        let url = URL(fileURLWithPath: path, isDirectory: true)
        return fileAvailability.fileExists(at: url) ? url : nil
    }

    private func updateMixReviewArtifactFolder(_ folderURL: URL) {
        settingsActions.updateMixReviewArtifactFolderPath(folderURL.path)
    }
    ...
    func restoreMixReviewArtifacts() {
        do {
            let state = try artifactStore.load()
    ...
    var selectedMixReviewPair: PlannedMixPair? {
        if let selectedTrack = library.selectedTrack, let nextTrack = library.nextAvailableTrack {
            return PlannedMixPair(currentTrackId: selectedTrack.id, nextTrackId: nextTrack.id)
        }
        return planning.currentPair
    }
    ...
    private func trackTitle(for trackId: String) -> String? {
        if let imported = library.playlist.first(where: { $0.id == trackId }) {
