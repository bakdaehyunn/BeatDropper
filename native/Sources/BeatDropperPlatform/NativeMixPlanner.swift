...
public struct NativeMixPlannerBridge: Sendable {
    public var timeoutSec: TimeInterval = 45

    public init(timeoutSec: TimeInterval = 45) {
        self.timeoutSec = timeoutSec
    }

    public func requestMixPlan(
    ...
    public func requestMixPlan(
        ...
        var configured = self
        configured.timeoutSec = max(1, timeoutSec)
        return await configured.requestMixPlan(
            request: request,
            validationContext: validationContext
        )
    }
