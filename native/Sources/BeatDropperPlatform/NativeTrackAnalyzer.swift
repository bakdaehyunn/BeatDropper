...
public enum NativeTrackAnalyzer {
    public static func analyze(track: Track, url: URL) throws -> TrackAnalysis {
        let pcmBuffer = try readPCMBuffer(from: url)
        return NativeDSPAnalyzer().analyze(
            track: track,
            buffer: pcmBuffer,
            fileRevision: try currentFileRevision(for: url)
        )
    }

    public static func currentFileRevision(for url: URL) throws -> TrackFileRevision {
