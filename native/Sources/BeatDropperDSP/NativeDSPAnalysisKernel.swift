...
/// Deterministic, allocation-tolerant analysis math shared by the cohesive
/// pipeline stages. This kernel has no I/O, platform, persistence, or UI
/// dependencies and is intentionally not abstracted behind protocols.
struct NativeDSPAnalysisKernel: Sendable {
    init() {}

    private static let waveformBuckets = 160
    static let waveformDetailMaxBuckets = 1_200
    ...
    func resolveDuration(track: Track, buffer: PCMAnalysisBuffer) -> Double {
    ...
    func buildWaveformPeaks(samples: [Float], durationSec: Double) -> [WaveformPeak] {
    ...
    func buildEnergyProfile(samples: [Float], durationSec: Double) -> [Double] {
        guard !samples.isEmpty, durationSec > 0 else {
            return []
        }
    ...
    func buildWaveformDetail(samples: [Float], durationSec: Double) -> [WaveformDetailPoint] {
        guard !samples.isEmpty, durationSec > 0 else {
            return []
        }
    ...
    func buildLoudnessAnalysis(buffer: PCMAnalysisBuffer, durationSec: Double) -> LoudnessAnalysis? {
    ...
    func buildStereoAnalysis(buffer: PCMAnalysisBuffer) -> StereoAnalysis? {
        let channels = buffer.channelSamples.filter { !$0.isEmpty }
        if channels.count < 2 {
    ...
    func estimateBPM(transientMarkers: [TransientMarker]) -> (bpm: Double?, confidence: Double) {
    ...
    func buildBeatGrid(durationSec: Double, beatIntervalSec: Double, beatOffsetSec: Double) -> [Double] {
    ...
    func scoreTransientMarkerQuality(
        transientMarkers: [TransientMarker],
        beatGridSec: [Double],
        durationSec: Double,
    ...
    func resolveOutroCueSec(barGrid: [BarMarker], durationSec: Double) -> Double? {
    ...
    func snapToNearestBar(barGrid: [BarMarker], timeSec: Double) -> Double? {
